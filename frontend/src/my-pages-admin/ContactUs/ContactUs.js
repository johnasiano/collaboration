...
const ContactUs = () => {
  ...
  useEffect(() => {
    AdminService.getOptions("contact_us").then((data) => {
      setText(data.text);
    })
  }, []);
  ...
  const handleUpdateContact = () => {
    ...
    AdminService.setOptions(data)
      ...
      .catch((error) => {
        cogoToast.error('Failed to update terms service', { position: 'top-right'});
      })
  }
    ...
    <Fragment>
      ...
      >
        <Grid
          ...
          justify="flex-end"
        >
          <Button variant="contained" color="primary" className="m-2" onClick={handleUpdateContact} >
