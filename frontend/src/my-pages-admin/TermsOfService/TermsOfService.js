...
const TermsOfService = () => {
  ...
  useEffect(() => {
    AdminService.getOptions("terms_service").then((data) => {
      setText(data.text);
    })
  }, []);
  ...
  const handleUpdateTermservice = () => {
    if (text === '') return;
    const data = {
      option_name: "terms_service",
      option_value: {
        text: text
      }
    }
    ...
    <Fragment>
      ...
      >
        <Grid
          ...
          justify="flex-end"
        >
          <Button variant="contained" color="primary" className="m-2" onClick={handleUpdateTermservice} >
