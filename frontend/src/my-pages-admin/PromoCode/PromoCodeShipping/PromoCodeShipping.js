...
const PromoCodeShipping = () => {
  ...
  useEffect(() => {
    AdminService.getShippingPromo().then((data) => {
      setPromoCodes(data);
    })
  }, []);
  ...
  const handleUploadPromo = () => {
    ...
    if (promoFile) {
      ...
      setPromoCodes(null);
      AdminService.uploadShippingPromoCodes(uploadData).then(data => {
  ...
  return (
    <Fragment>
      <PageTitle
          titleHeading="Promo with Shipping Setup"
...
export default PromoCodeShipping;
