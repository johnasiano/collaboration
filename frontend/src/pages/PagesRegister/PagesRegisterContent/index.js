...
import { signupAccount, signupAccountGoogle } from 'services/auth.service';
...
const Register = () => {
  ...
  const onSuccess = (res) => {
    ...
    signupAccountGoogle(accountInfo).then((result) => {
      if (result.success) {
        dispatch(login({
          user: result.user,
  ...
  const onFailure = (res) => {
    cogoToast.error('Failed google register', { position: 'top-right'});
  };
  ...
  const handleSubmit = async event => {
    ...
    signupAccount(accountInfo).then((result) => {
      if (result.success) {
        router.history.push('/user/login');
        cogoToast.success('Successfully register!', { position: 'top-right'});
      } else {
        cogoToast.error(result.message, { position: 'top-right'});
      }
    })
                      ...
                      <Grid
                        ...
                        <span className="w-100 text-left text-md-center pb-4">
                          ...
                          <p className="font-size-lg text-xl-left text-center mb-0 text-black-50">
                            We're glad you're working on your app. Register below
                            to continue.
                          </p>
                        </span>
